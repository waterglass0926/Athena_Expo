      ...
      <View style={styles.container}>
        ...
        {results && results.length > 0 && (
          <View
            ...
            <Components.SCText>{results.length} Results</Components.SCText>
          </View>
        )}
        <View
          style={[
          ...
          {!results && !loadingResults && (
            <>
              ...
              <FlatList
                ...
                ItemSeparatorComponent={Components.ListItemSeparator}
                data={recentSearches}
                renderItem={({ item, index }) => {
                  return (
                    <TouchableOpacity
                      ...
                      <Components.SCText style={styles.recentSearchText}>{item}</Components.SCText>
                      <Components.SCText
                        onPress={() => {
                ...
                ListEmptyComponent={() => {
                  return (
                    <View style={styles.listEmptyContainer}>
                      <Components.SCText>No results for '{searchText}'</Components.SCText>
                      <TouchableOpacity
                        onPress={startNewSearch}
                        style={styles.resetButton}>
                        <Components.SCText>Start a new search</Components.SCText>
                      </TouchableOpacity>
                    </View>
                  );
                }}
                ...
                renderItem={({ item }) => {
                  return (
                    <TouchableOpacity
                      ...
                      <Components.SCText style={styles.resultChannelTitle}>
                        {getChannelDisplayName(item.channel, true)}
                      </Components.SCText>
                      <ThemeProvider style={chatStyle}>
                        <DefaultMessage
                          Message={props => (
                            <Components.MessageSlack
                              {...props}
                              onPress={() => {
                                navigation.navigate(
                                  'TargettedMessageChannel',
                                  {
                                    message: item,
                                  },
                                );
