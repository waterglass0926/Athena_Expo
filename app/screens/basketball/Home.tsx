...
class Basketball extends Component {
  ...
  renderNet(render) {
    if (render === true) {
      return (
        <Components.Net y={NET_Y} x={NET_X} height={NET_HEIGHT} width={NET_WIDTH} />
      );
    }
    return null;
  }
  ...
  renderFloor(render) {
    ...
    if (render === true) {
      return (
        <Components.Floor height={FLOOR_HEIGHT} />
      );
    }
    return null;
  }
  ...
  render() {
    return (
      <View style={styles.container}>
        <Components.Score y={FLOOR_HEIGHT * 3} score={this.state.score} scored={this.state.scored} />
        <Components.Hoop y={HOOP_Y} />
        ...
        {this.renderNet(this.state.lifecycle !== LC_STARTING)}
        {this.renderFloor(this.state.vy > 0)}
        <Components.Emoji y={NET_Y} scored={this.state.scored} />
      </View>
    );
  }
}
